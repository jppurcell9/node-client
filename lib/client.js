  ...
  request: function(options) {
    ...
    return when.promise(function(resolve, reject) {
      ...
      var request = http.request(requestOptions, function(res) {
        ...
        res.on("end", function() {
          console.log('response.on(end)...', res.statusCode);
          if (noDataTimeout) {
            clearTimeout(noDataTimeout);
            noDataTimeout = null;
          }
      ...
      var tryingCurl = false;
      noDataTimeout = setTimeout(function() {
        console.log('received no data within 10 seconds, trying curl');
        tryingCurl = true;
        request.abort();

        var args = [
          '-sS',
          '-X', requestOptions.method.toUpperCase()
        ];
        ...
        curl.on('close', (code) => {
          console.log('curl exited with code', code, typeof code);
          if (code === 0) {
            console.log(resultBody);
            resolve({client: client, data: JSON.parse(resultBody), meta: {}});
          } else {
            reject({client: client, data: resultBody, meta: {}});
          }
        });
      ...
      request.on("error", function(err) {
        console.log('request.on(error)...', err);
        if (tryingCurl) {
          console.log('trying curl instead.');
          return;
        }
        if ((requestOptions.method == "get" ||
             requestOptions.method == "put" ||
